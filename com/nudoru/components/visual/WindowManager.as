package com.nudoru.components.visual
{
	 ...
	 */
	public class WindowManager extends VisualComponent implements IWindowManager
	{
		...
		public function showMessage(title:String, message:String, emphasis:Boolean = false, height:int = 250, modal:Boolean = false):IWindow
		{
			...
			window.initialize({width:400, height:height, title:title, content:message, font:"Verdana", size:12, leading:5, color:0x000000, emphasis:emphasis, modal:modal, buttons:[Window.closeButton]});
			window.render();
		...
		protected function doWindowInAnimation(window:DisplayObject):void
		{
			...
			TweenMax.to(window, .75, {alpha:1, y:targetY, ease:Expo.easeOut});
		}
