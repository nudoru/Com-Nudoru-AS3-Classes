package com.nudoru.utilities
{
	...
	public class HTMLContainerUtils
	{
		...
		public static function getQueryStringValue(parameter:String, source:String):String
		{
			...
			if(match.length) return match[0].split("=")[1];

			// maybe the parameter is all caps?
			regexp = new RegExp("[?&]"+parameter.toUpperCase()+"(?:=([^&]*))?", "gi");
			match = source.match(regexp);
			if(match.length) return match[0].split("=")[1];
			
			// maybe the parameter is all lower?
			regexp = new RegExp("[?&]"+parameter.toLowerCase()+"(?:=([^&]*))?", "gi");
			match = source.match(regexp);
			if(match.length) return match[0].split("=")[1];
