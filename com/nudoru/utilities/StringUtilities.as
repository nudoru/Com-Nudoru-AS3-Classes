package com.nudoru.utilities
{
	...
	/**
	 * StringUtilities provides several static methods to handle various string related tasks. 
	 */
	public class StringUtilities
	{
		...
		/**
		 * A slug is usually a few words with each word separated by a delimiting character, 
		 * usually an underscore or hyphen. A slugified version of the string “10 Tips to a 
		 * Better Life!” would be “10-tips-to-a-better-life”.
		 * This function takes an arbitrary string and converts in for use as a slug. We use two regular 
		 * expressions to accomplish this, the first is used to remove any character which is not a word 
		 * character, space or hyphen, the second replaces any spaces with a single hyphen. Finally we 
		 * convert the string to lowercase. This returns a new string, leaving the input string unchanged.
		 * 
		 * From http://active.tutsplus.com/articles/roundups/15-useful-as3-snippets-on-snipplr-com/
		 */
		public static function slugify(string:String):String
		{
			...
			// Matches anything except word characters, space and -
		...
		/**
		 * Via Nutrox - https://gist.github.com/778392
		 */
		...
		/**
		 * Compress a string as a ByteArray. 
		 * <p>Provided via <a href='http://www.flexdeveloper.eu/forums/actionscript-3-0/compress-and-uncompress-strings-using-bytearray/'>flexdeveloper.eu forums</a></p>
		 * @param string the string to convert to a compressed byteArray.
		 * @return the compressed string as a byteArray.
		 */
		public static function compress(string:String):ByteArray
		{
		...
		/**
		 * Decompress a string as a ByteArray. 
		 * <p>Provided via <a href='http://www.flexdeveloper.eu/forums/actionscript-3-0/compress-and-uncompress-strings-using-bytearray/'>flexdeveloper.eu forums</a></p>
		 * @param	byteArray the byteArray to decompress
		 * @return a string of the decompressed byteArray
		 */
		public static function decompress(byteArray:ByteArray):String
		{
			try
			{
			...
			{
				trace("Error decompressing data: " + error);
			}
			return byteArray.toString();
		}

		/**
		 * Compresses a string to a ByteArray and then encodes via Base64. Returns a string that may be saved
		 * @param	str
		 * @return
		 */
		public static function compressBase64(str:String):String
		{
		...
		/**
		 * Decompresses a Base64/ByteArray compressed string
		 * @param	str
		 * @return
		 */
		public static function decompressBase64(str:String):String
		{
		...
		/**
		 * Converts a string representation of a boolean to a true boolean value.
		 * For example, it will convert "TRUE", "False", "true", "TruE", "f", or "T" to a real boolean (true or false).
		 * The input string is not case-sensitive.  Acceptable arguments are the word "true" or "false", or the letter "t" or "f".
		 * @param value the string to convert to a boolean.
		 * @return representation of the string as a true boolean value.
		 **/
		public static function stringToBoolean(value:String):Boolean
		{
			var boolean:Boolean = false;
			if(value.toLowerCase() == "true") boolean = true;
			if(value.toLowerCase() == "t") boolean = true;
			return boolean;
		}

		/**
		 * Converts seconds to a string representing minutes and seconds.
		 * This is useful to convert, for example 132 seconds to "2 minutes, 12 seconds".
		 * @param	totalSeconds the total seconds to convert to minutes and seconds.
		 * @return returns a string in the format of "X minutes, Y seconds".
		 **/
		public static function getMinutesSecondsString(totalSeconds:int):String
		{
			var numMinutes:int = Math.floor(totalSeconds / 60);
			var remainingSeconds:int = totalSeconds - (numMinutes * 60);
			return numMinutes + " minutes, " + remainingSeconds + " seconds";
		}
		...
		public static function stripEditor(value:String):String
		{
			return value.replace(/([0-9;#])/g, "");
		}

		public static function stripPunctuation(value:String):String
		{
			return value.replace(/.,/g, "");
		}

		public static function replaceWhiteSpace(value:String, replace:String = ""):String
		{
		...
		/**
		 * Removes <font face="x"></font> tags
		 * @param	$s	String to remove tags from
		 * @return	A string with the tags stripped
		 */
		public static function removeFontTagsText($s:String):String
		{
			var s:String = escape($s);
			var fontPat1:RegExp = /%3Cfont%20face%3DArial%3E/gi;
			var fontPat2:RegExp = /%3Cfont%20face%3DVerdana%3E/gi;
			var fontPat3:RegExp = /%3Cfont%20face%3DCalibri%3E/gi;
			var entFontPat:RegExp = /%3C\/font%3E/gi;
			var string1:String = s.replace(fontPat1, "");
			var string2:String = string1.replace(fontPat2, "");
			var string3:String = string2.replace(fontPat3, "");
			var string4:String = string3.replace(entFontPat, "");
			return unescape(string4);
		}
